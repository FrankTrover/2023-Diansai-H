...
原题给出的流程图包含 FPGA、寄存器组和双 DAC 等模块，与本设计实际硬件不一致。本设计流程图改为图 \ref{fig:system_block} 所示。
...
加法器为独立电路板，与分离电路只连接混合信号 $C$ 和地线。输入 $A$、$B$ 经过 TL072 运放构成的缓冲与求和电路，降低信号源与求和网络之间的相互影响，再通过等阻值电阻网络实现单位权重求和。图 \ref{fig:adder} 为加法器原理示意。若实际 PCB 采用反相求和结构，则输出整体反相不影响频率识别与后续 DDS 重构；需要严格满足 $C=A+B$ 极性时，可在后级增加一级反相或改为同相求和拓扑。
...
\noindent\textbf{供电说明：}
TPS5430 产生的正负 5 V 主要为 TL072 运放及加法器模拟前端服务，不作为 AD7616 的正负模拟输入电源使用。TPS5450 提供系统正 5 V，用于 DDS 模块、LCD 背光和后级模块，并可经后级稳压获得数字 3.3 V。
...
输出端采用两片独立 DDS。AD9833 负责低频分量 $A'$，AD9834 负责高频分量 $B'$。两片 DDS 均使用 GPIO 模拟 SPI 写入 16 位控制字，互不共用片选和数据线。DDS 原始输出再接后级放大模块，调整幅度到不小于 1 Vpp，并提供 $A'$、$B'$ 测试端。后级放大电路如图 \ref{fig:output_amp} 所示。
...
\end{equation}
定时器更新中断中等待 AD7616 的 BUSY 拉低，然后拉低 CS/RD，直接读取 GPIOD->IDR 的 16 位数据。与逐位调用 HAL\_GPIO\_ReadPin 相比，寄存器读法显著降低采样开销，保证 500 kSPS 下能完成每点读取。
...
\begin{enumerate}
  ...
  \item 调用 CMSIS-DSP 的 2048 点复数 FFT。
  \item 在 20 kHz--100 kHz 范围内寻找两个主谱峰。
  \item 对主谱峰做抛物线插值，估计更精确的频率。
  \item 检查三次、五次谐波幅值比例，判别正弦波或三角波。
...
从理论分辨率看，FFT 频率 bin 间隔约为 244 Hz，远小于题目的 5 kHz 最小频率栅格。DDS 频率分辨率低于 1 Hz，频率重构误差远小于 FFT 识别误差。若实测出现漂移，优先检查频率栅格吸附是否启用、DDS 主频常量是否与模块晶振一致，以及后级放大模块是否引入削顶失真。

\section{改进方案}

本设计当前主控为 STM32F407，主频 168 MHz，已能完成 500 kSPS、2048 点 FFT 和双 DDS 控制。后续可将主控升级为 STM32F722。F722 采用 Cortex-M7 内核，主频可达 216 MHz，具有更强的 DSP 运算能力、缓存结构和更高的总线吞吐能力。升级后可考虑以下改进：
\begin{itemize}
  \item 提高 FFT 点数到 4096 或 8192，提升频率估计稳定性。
  \item 引入 DMA/FMC 等更适合并行数据搬运的采样结构，减轻中断负担。
  \item 增加相位估计与 DDS 相位字输出，使 $A'$、$B'$ 与原信号保持可控相位差。
